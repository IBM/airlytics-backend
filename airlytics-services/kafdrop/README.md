...
This project is a fork of [obsidiandynamics/kafdrop](https://github.com/obsidiandynamics/kafdrop) that adds topic lag reporting to Prometheus. It only adds the following to the original project (v.3.28.0):
* Prometheus dependencies in pom.xml
* Dockerfile
* **kafdrop.config.SchedulingConfiguration** Spring configuration class
* **kafdrop.controller.MetricsController** Spring REST controller class adding `/metrics` endpoint for Prometheus
* **kafdrop.service.PrometheusMetricsService** Spring service class periodically reporting lags to Prometheus metrics
