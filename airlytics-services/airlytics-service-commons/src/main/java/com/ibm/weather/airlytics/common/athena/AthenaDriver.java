...
public class AthenaDriver {
    ...
    private static final RateLimiter queryLimiter = RateLimiter.create(1.0);

    private static final RateLimiter executionLimiter = RateLimiter.create(20.0);

    public AthenaClient getAthenaClient(BasicAthenaConfig featureConfig) throws AirlockException {
        return AthenaClient.builder()
                .region(getRegion(featureConfig))
                .build();
    }
    ...
    public String submitAthenaQuery(
            ...
            BasicAthenaConfig featureConfig) throws AthenaException {
        queryLimiter.acquire();

        try {
            // The QueryExecutionContext allows us to set the database
            QueryExecutionContext queryExecutionContext = QueryExecutionContext.builder().catalog(featureConfig.getAthenaCatalog()).build();

            // The result configuration specifies where the results of the query should go
            ResultConfiguration resultConfiguration = ResultConfiguration.builder()
                    .outputLocation("s3://" + featureConfig.getAthenaResultsOutputBucket() + "/")
                    .build();
    ...
    private Region getRegion(BasicAthenaConfig featureConfig) throws AirlockException {
