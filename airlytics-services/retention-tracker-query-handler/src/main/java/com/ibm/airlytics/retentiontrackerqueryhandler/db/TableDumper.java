...
public class TableDumper {
    ...
    public TableDumper(TableDumpConfig tableConfig, DbHandler dbHandler, AthenaHandler athenaHandler, boolean forComplete) throws IOException {
    ...
    public void setEXTRA_PREFIX(String EXTRA_PREFIX) {
        this.EXTRA_PREFIX = EXTRA_PREFIX;
    }

    private String getConvertSelectPart(TableDumpConfig tableConfig) throws SQLException, ClassNotFoundException {
    ...
    private String buildParquetSchema(TableDumpConfig tableConfig) throws SQLException, ClassNotFoundException {
        String schema = tableConfig.getSchemaName();
        String table = tableConfig.getTableName();
    ...
    private void addFieldToConvertSelect(String fieldName, FieldConfig fieldConfig, StringBuilder sb) {
        switch (fieldConfig.getType()) {
            ...
            case LONG:
                ...
                break;
            case NUMBER:
                String customN = tableConfig.getCasts().get("NUMBER");
                String numTemplate = "CAST(%s AS BIGINT) as %s";
                if (customN != null && !customN.isEmpty()) {
                    numTemplate = customN;
                }
    ...
    public TableDumpResult dumpTable(String day, String snapshotPath) throws InterruptedException, SQLException, ClassNotFoundException, IOException {
        String successFilePath = composeSuccessFilePath(day);
        String successFile = destDataSerializer.getFileContent(successFilePath);
        TableDumpResult res = new TableDumpResult();
        if (successFile != null) {
            logger.info("skipping table "+tableConfig.getSchemaName()+"."+tableConfig.getTableName()+" already written for day "+day);
            return res;
        }
        ...
        try {
            if (tableConfig.getViewTables().size() > 0) {
            ...
            if (tableConfig.getSharded()) {
                ...
                if (tableConfig.isDumpShardNull()) {
                    Callable<Result> w = new ShardConvertTask(day, tableName, snapshotPath, select, -1, true, tableConfig.getSharded(), tableConfig.getLatestOnly());
                ...
                for (int shard=0; shard<numShards; shard +=multiShardNum) {
                    Callable<Result> w = new MultiShardConvertTask(day, tableName, snapshotPath, select, shard, shard+multiShardNum, false, tableConfig.getSharded(), tableConfig.getLatestOnly());
                    Future<Result> future = executor.submit(w);
                    writings.add(future);
                }
            } else {
                Callable<Result> w = new ShardConvertTask(day, tableName, snapshotPath, select, 0, false, tableConfig.getSharded(), tableConfig.getLatestOnly());
                Future<Result> future = executor.submit(w);
                writings.add(future);
            }
            ...
            if (!errors.isEmpty()) {
                logger.error("errors during writing of "+tableConfig.getSchemaName()+"."+tableConfig.getTableName()+": " + errors.toString());
                logger.error("didnot finish writing "+tableConfig.getSchemaName()+"."+tableConfig.getTableName()+" for day "+day);
                res.error = errors.toString();
                String errorFilePath = composeErrorFilePath(day);
                destDataSerializer.writeData(errorFilePath, errors.toString());
            } else {
                //write success file
                destDataSerializer.writeData(successFilePath, "");
            }
            logger.info("finished dumping table "+tableConfig.getSchemaName()+"."+tableConfig.getTableName()+" in "+timer.getTotalTime()/60.0+" minutes");
    ...
    private String composeSuccessFilePath(String dayPath) {
        ...
        sb.append("_SUCCESS_"+dayPath);
        return sb.toString();
    }

    private String composeErrorFilePath(String dayPath) {
        StringBuilder sb = new StringBuilder(tableConfig.getS3RootFolder()+File.separator);
        sb.append("_FAIL_"+dayPath);
        return sb.toString();
    }
    ...
    class MultiShardConvertTask implements Callable {
        ...
        public MultiShardConvertTask(String day, String athenaTableName, String snapshotPath, String select, int minShard, int maxShard, boolean isShardNull, boolean sharded, boolean isLatest) {
            this.day = day;
        ...
        public Result call() throws InterruptedException, IOException {
            ...
            try {
                ...
                for (int shard=minShard; shard<maxShard; ++shard) {
                    String destDay = this.isLatest? Constants.LATEST : day;
                    String destinationFolder = composeFilePath(tableConfig, shard, destDay);
                ...
                if (!forComplete) {
                    ...
                    for (Result currRes : results) {
                        if (currRes.error != null) {
                            errors.add(currRes.filePath + ": " + currRes.error);
                        } else {
                            paths.add(currRes.filePath);
                        }
                    }
                }
        ...
        private String composeTempFolder(TableDumpConfig tableConfig, String athenaTableName, int minShard, int maxShard, String day) {
            StringBuilder sb = new StringBuilder("s3://" +tempS3FilePrefix+EXTRA_PREFIX);
        ...
        private String composeFilePath(TableDumpConfig tableConfig, int shard, String day) {
            StringBuilder sb = new StringBuilder(tableConfig.getS3RootFolder()+File.separator);
            if (tableConfig.getSharded()) {
    ...
    class CopyFilesTask implements Callable {
        ...
        public Result call() {
            ...
            try {
                ...
                logger.error("error during copying temp files to"+destinationFolder);
                res.error = "error during copying temp files to"+destinationFolder;
            }
            return res;
        }
    }
    class ShardConvertTask implements Callable {
        ...
        public ShardConvertTask(String day, String athenaTableName, String snapshotPath, String select, int shard, boolean isShardNull, boolean sharded, boolean isLatest) {
            this.day = day;
            this.athenaTableName = athenaTableName;
            this.snapshotPath = snapshotPath;
            this.shard = shard;
...
//                String destinationFolder = composeFilePath(tableConfig, shard, day);
                ...
                if (!this.sharded) {
                    ...
                    location = athenaHandler.convertAthenaTableForShardNull(athenaTableName, shard, tempDestinationFolder, select, tableConfig.getOrderByField(), tableConfig.getSchemaName(), tableConfig.getTableName());
                } else {
...
//                String destinationFolder = StringUtils.removeStart(composeFilePath(tableConfig, shard, day),config.getS3Bucket()+File.separator);
                ...
                String destDay = this.isLatest? Constants.LATEST : day;
                String destinationFolder = composeDestFilePath(tableConfig, shard, destDay);
                CopyFilesTask task = new CopyFilesTask(locationPrefix, day, destinationFolder, shard);
                Future<Result> future = this.executor.submit(task);
                Result result = future.get();
                logger.debug("finished readi ng copy results of:"+result.filePath);
                if (result.error != null) {
        ...
        private String composeTempFolder(TableDumpConfig tableConfig, String athenaTableName, int shard, String day) {
            StringBuilder sb = new StringBuilder("s3://" +tempS3FilePrefix+EXTRA_PREFIX);
            sb.append(tableConfig.getSchemaName()+"."+tableConfig.getTableName()+File.separator);
            sb.append("day=" + day+File.separator);
        ...
        private String composeSourceFolder(TableDumpConfig tableConfig, String athenaTableName, int shard, String day) {
            StringBuilder sb = new StringBuilder("s3://" +tempS3FilePrefix+EXTRA_PREFIX);
            sb.append(tableConfig.getSchemaName()+"."+tableConfig.getTableName()+File.separator);
            sb.append("day=" + day);
            return sb.toString();
        ...
        private String composeDestFilePath(TableDumpConfig tableConfig, int shard, String day) {
            StringBuilder sb = new StringBuilder(tableConfig.getS3RootFolder()+File.separator);
            if (tableConfig.getSharded()) {
                sb.append("shard=" + shard + File.separator);
            }
    ...
    public TableDumpResult dumpTable(String day) throws SQLException, ClassNotFoundException {
        TableDumpResult res = new TableDumpResult();
        logger.info("Dumping table "+tableConfig.getTableName());
        TrackerTimer timer = new TrackerTimer();
        parquetSchema = buildParquetSchema(tableConfig);
        //iterate over all relevant shards
        ArrayList<Future<Result>> writings = new ArrayList<Future<Result>>();
        ...
        if (tableConfig.isFromAthena()) {
            if (tableConfig.getSharded()) {
                ...
                int steps = tableConfig.getShardBatchSize();
                for (int shard=0; shard<numShards; shard = shard+steps) {
                    Callable<Result> w = getAthenaWriteTask(shard, shard+steps, day);
                    Future<Result> future = executor.submit(w);
                    writings.add(future);
                }
            } else {
                Callable<Result> w = getWriteTask(0, day, tableConfig.isFromAthena());
                Future<Result> future = executor.submit(w);
                writings.add(future);
            }

        }
        else if (tableConfig.getSharded() == true) {
            int numShards = tableConfig.getShardsNumber();
            if (numShards <= 0) {
                numShards = config.getShardsNumber();
            }
            if (tableConfig.isDumpShardNull()) {
                Callable<Result> w = getWriteTask(tableConfig.getShardNullNumber(), day, true, tableConfig.isFromAthena());
                Future<Result> future = executor.submit(w);
                writings.add(future);
            }
            for (int shard=0; shard<numShards; ++shard) {
                Callable<Result> w = getWriteTask(shard, day, tableConfig.isFromAthena());
                Future<Result> future = executor.submit(w);
                writings.add(future);
            }
        } else {
    ...
    class S3ParquetFromAthenaWriteTask implements Callable {
        ...
        private CustomParquetWriter<Group> getWriter(TableDumpConfig tableConfig, int shard, String day, MessageType schema) throws IOException {
                    ...
                    try {
                        for (GetQueryResultsResponse result : resultIterable) {
                            ...
                            for (Row currRow : results) {
                                ...
                                ParquetDumper.filesWrittenCounter.labels(RESULT_SUCCESS).inc();
                                ParquetDumper.filesSizeBytesSummary.observe(writer.getDataSize());
                            }
                        }
                        ...
                        ParquetDumper.filesWrittenCounter.labels(writeResult).inc();
                    }
            } catch (Exception e) {
        ...
        private String composeFilePath(TableDumpConfig tableConfig, int shard, String day, long firstRecordOffset) {
                ...
                while (retryCounter < config.getWriteRetries()) {
                    ...
                    try {
                        ...
                        while (rs.next()) {
                            ...
                            ParquetDumper.recordsProcessedCounter.labels(recordResult).inc();
                        }
